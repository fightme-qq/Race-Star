...
const SPACE = {
  ...
  // Верхняя граница поднята на Этапе 4: подбор встал на 187K при потолке 200K,
  // то есть хотел ослабить множитель фанатов ещё сильнее, а граница держала
  // молча. Третий раз подряд одна и та же грабля — прижатую границу надо
  // проверять КАЖДЫЙ прогон, а не замечать по счёту.
  fansK:      [200, 2000000],
  leagueMult:   [1.2, 3.0],
...
const SEED_POINTS = [
  ...
  // Точка Этапа 3 как она легла в конфиг. Этап 4 переписал ядро заезда и
  // починил распределение мест, поэтому она перестала попадать: счёт 0.405
  // против 0.130 до правки. Держим ориентиром — экономика уезжать далеко не
  // должна, ехать обязана боевая часть.
  { ecoGain: 0.3421, ecoPg: 2.2900, fanGain: 0.5320, fanPg: 1.2510,
    fansK: 173119, leagueMult: 1.3505, leagueStep: 1.4183, promoteRatio: 1.9136 },
  // Первый прогон Этапа 4, счёт 0.169 — на нём и вскрылась прижатая fansK.
  { ecoGain: 0.3513, ecoPg: 2.2900, fanGain: 0.4533, fanPg: 1.2510,
    fansK: 187537, leagueMult: 1.3505, leagueStep: 1.4183, promoteRatio: 1.9653 },
]
