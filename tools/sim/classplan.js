...
// бесплатна: фанаты, лига, карьерный драйвер и уровни апгрейдов у каждого
// класса свои и обнуляются, доход падает к полу $1/с [F], а взамен приток
// фанатов идёт множителем classFanMult^index. Это классическая развилка идлера
// («престиж»): рано — потеряешь накопленное, поздно — не успеешь разогнать.
...
function rates(state, target) {
  ...
  return {
    ...
    cur: { rate, fans: state.cls.fans, prize },
    next: { rate: rate * step, fans: 0, prize },
  }
}
...
const project = (side, R) =>
  (side.fans * R + side.rate * R * R / 2) * RACE.durationSec
  / ECONOMY.fansPerDollar * (1 + side.prize)
