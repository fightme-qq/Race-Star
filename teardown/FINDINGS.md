...
`Counter Force` (Defense конвертится в Offense) в дерево **не вошёл**, и это
не забывчивость. Наша модель заезда ранжирует по СУММЕ offense+defense:
пошаговый шум, который гасит защита, за 600 тиков усредняется до ±0.3% и на
исход не влияет. То есть перенос защиты в атаку — тождественная операция, узел
был бы мёртвым (прогон: 6 очков в него стоят −3.6% дохода, ровно как 6 очков в
никуда). Из этого же следует, что и деление боевых апгрейдов на «атакующие» и
«защитные» у нас пока косметическое, хотя в оригинале оно есть [F]. Чтобы
вернуть и узел, и смысл деления, защита должна влиять на **разброс** результата,
а не только на сумму — это отдельная правка ядра гонки, не сделана.
