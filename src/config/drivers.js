...
// [F] секция состава подписана «STARTERS (5)» с меткой PLAYING NOW — и это
// ОПРОВЕРГНУТО шагом 0: на планшетном кадре стоит `STARTERS (9)`, а в билде
// строка параметризована — `STARTERS ({0})` [E]. Значит слотов больше пяти и
// они открываются; чем именно — в кадрах нет.
// SQUAD_SIZE остался, но теперь это РОВНО стартовая пятёрка: столько драйверов
// выдаётся при открытии класса, и столько же строк в STARTER_STATS, на сумме
// которых стоит точка калибровки экономики.
export const SQUAD_SIZE = 5

// Сколько слотов состава доступно. [E] из трёх шаблонов условий разблокировки
// (`Play {0} races`, `Win {0} race`, `Own {0} teams`) берём третий: в билде
// размер состава — НАЗВАННЫЙ СТАТ `Seat Count` [E], то есть он растёт от
// прогресса игрока в целом, а не от пробега в одном классе.
// Числа [X]: +1 слот за каждый открытый класс сверх первого, то есть 5 на
// старте и 10 к шестому классу. Наблюдённая девятка приходится на пятый класс.
// Новый слот НЕ заполняется автоматически стартовым драйвером: иначе открытие
// класса печатало бы силу из воздуха (та же грабля, что в правиле 22).
export const SQUAD_MAX = 10
export const seatCount = (classesOwned) =>
  Math.min(SQUAD_MAX, SQUAD_SIZE + Math.max(0, classesOwned - 1))
