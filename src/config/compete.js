...
export const BRACKET = {
  ...
  //
  // ПЯТЬ, а не двадцать, и это не вкусовщина. Период турнира лиги — СЕЗОН
  // класса, а сезон это ровно SEASON.races = 20 заездов: при двадцати первый
  // раунд становился доступен в тот же заезд, когда сезон переворачивался, ключ
  // менялся, и сетка сбрасывалась в «не зарегистрирован». Замер: регистрация,
  // 45 заездов -> round 0, place null, лог пуст. Полуфинала турнир лиги не
  // видел никогда. Три раунда по пять укладываются в сезон с запасом.
  racesPerRound: 5,
}
...
export const CUP = {
  ...
  // У кубка раунды ДЛИННЕЕ, и он может себе это позволить: его период — ступень
  // (tier.attempt), а не сезон, поэтому сетка не сбрасывается под ним.
  // Ступень кубка должна ощущаться дольше сезонного турнира, иначе два экрана
  // сливаются в один.
  racesPerRound: 20,        // [X]
}
